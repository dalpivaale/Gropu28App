...
class FetchPage extends StatelessWidget {
  ...
  FetchPage({Key? key, required this.selectedDatePace,required this.obbiettivo}) : super(key: key);
  ...
  @override
   ...
   List <double?> stepsvalue= [null,null,null,null,null,null,null]; //Lists for build value graphic's axis
  ...
  String? userId = null;
   Future<void> _authorized() async {
      ...
      for (var i = 0; i < 7; i++) {
        stepsvalue[i] = stepsData[i].value;
        datevalue[i] = stepsData[i].dateOfMonitoring;
      }
      print('\n');
      print(stepsvalue);
      print(datevalue);
    ...
    _authorized();
      ...
      return Consumer <Cart>(
            builder: (context, cart, child) { 
                final chartData = _riempimento(cart.datevalue2, cart.stepsvalue2); //build chart with values fatched
                 return Scaffold(
                  appBar: AppBar(
    ...
    List<ChartSampleData> _riempimento(List<DateTime?> k, List<double?> j){
      ...
      for (var i = 0; i < 7; i++) { //build axis values chart
